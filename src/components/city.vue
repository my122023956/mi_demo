...
<div class="citybox">
    <div class="city">
        <div class="city0">
            <div class="city01" :style='city01css'  @touchstart='citytouchstart' @touchmove='citytouchmove' @touchend='citytouchend' id='city01'>
                <div v-for="(v,k) in this.city1" class="cityli" ref='city01' :class="{shoose1:k == -city01xb,shoose2:k == -city01xb+1 || k == -city01xb-1,shoose3:k == -city01xb+2 || k == -city01xb-2,shoose4:k == -city01xb+3 || k == -city01xb-3,shoose5:k == -city01xb+4 || k == -city01xb-4}">{{v}}</div>
            </div>         
        </div>
        <div class="city0">
            <div class="city02" :style='city02css'  @touchstart='citytouchstart' @touchmove='citytouchmove' @touchend='citytouchend' id='city02'>
                <div v-for="(v,k) in this.city2" class="cityli" ref='city02' :class="{shoose1:k == -city02xb,shoose2:k == -city02xb+1 || k == -city02xb-1,shoose3:k == -city02xb+2 || k == -city02xb-2,shoose4:k == -city02xb+3 || k == -city02xb-3,shoose5:k == -city02xb+4 || k == -city02xb-4}">{{v}}</div>
            </div>           
        </div>
        <div class="city0">
            <div class="city03" :style='city03css'  @touchstart='citytouchstart' @touchmove='citytouchmove' @touchend='citytouchend' id='city03'>
                <div v-for="(v,k) in this.city3" class="cityli" ref='city03' :class="{shoose1:k == -city03xb,shoose2:k == -city03xb+1 || k == -city03xb-1,shoose3:k == -city03xb+2 || k == -city03xb-2,shoose4:k == -city03xb+3 || k == -city03xb-3,shoose5:k == -city03xb+4 || k == -city03xb-4}">{{v}}</div>
            </div>
        </div>          
    </div>
    <div class="cityqd" @click='fcityqd'>
        <span>确定</span>
    </div> 
</div>
</template>
...
export default{
    data(){
        return{
            ...
            city2:[],//市级数组
            city3:[],//区级数组
            city01css:'transform: translate3d(0,84px,0);',
            city02css:'transform: translate3d(0,84px,0);',
            city03css:'transform: translate3d(0,84px,0);',
            city01xb:0,//省级当前值下标
            city02xb:0,//市级当前值下标
            city03xb:0,//区级当前值下标
            lineH:28,//每栏的高度     
    ...
    methods:{
        ...
        //更新市级列表
        updatacity2:function(xb){
            this.city2 = [];
            this.city02xb = 0;
            this.city03xb = 0;
            this.city02css = 'transform:translate3d(0px,84px,0px)';
            this.city03css = 'transform:translate3d(0px,84px,0px)';
            for( var k in this.cities[this.city1[xb]]){
                this.city2.push(k);
            }
            this.city3 = this.cities[this.city1[xb]][this.city2[0]];
        },
        //更新区级列表
        updatacity3:function(xb){
            this.city03xb = 0;
            this.city03css = 'transform:translate3d(0px,84px,0px)';
            this.city3 = [];
            this.city3 = this.cities[this.city1[-this.city01xb]][this.city2[xb]];
        },
        //确定选择
        fcityqd:function(){
            var str = this.city1[-this.city01xb]+' '+this.city2[-this.city02xb]+' '+this.city3[-this.city03xb];
            this.$emit('newAdd',str);
        }
    },
    mounted:function(){       
        //初始化省级数组
        ...
        //初始化当前省级的市级数组
        for(var k in this.cities[this.city1[0]]){
            this.city2.push(k);
        }
        //初始化当前市级的区级数组
        this.city3 = this.cities[this.city1[0]][this.city2[0]];      
